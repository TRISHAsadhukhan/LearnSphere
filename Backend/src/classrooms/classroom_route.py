...
# @task_route.get("/task/{task_id}", response_model=TaskResponseBody , status_code= status.HTTP_200_OK)
# def get_one_task_route(task_id:int, db : Session = Depends(get_db)):
#     return tasks_controller.get_one_task(task_id,db)
...
# ADD this route to src/classrooms/classroom_route.py
# Add it after the existing delete route

@classroom_route.patch("/regen-key/{class_id}", response_model=ClassroomResponseSchema, status_code=200)
def regenerate_room_key_route(
    class_id: int,
